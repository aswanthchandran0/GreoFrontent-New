...
const UserWithFollow:React.FC<Props>  = ({user,onClose})=>{
   ...
   const handleProfileView = ()=>{
    ...
    return(
        <div className="flex flex-row items-center p-2 my-2 rounded-md bg-background-charcoal">
          <div className="w-12 h-12 overflow-hidden rounded-full">
            <img src={user.profileImage || DEFAULT_PROFILE_IMAGE} alt="profile image" />
          </div>

          <div className="flex flex-col justify-center">
            <p className="text-text-white font-golos">{user.name || 'name'} </p>
            <p className="text-sm text-text-white font-golos">{user.user_name || 'username'}</p>
          </div>
